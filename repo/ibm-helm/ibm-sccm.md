...
[License](https://www.ibm.com/support/customer/csol/terms/?id=L-BGZS-2GMLUY&lc=en) (L/N: L-BGZS-2GMLUY)
...
```bash
$ helm install my-release -f values.yaml ibm-sccm-4.1.5.tgz
```
...
```bash
helm install <release-name> -f my-values.yaml ibm-sccm-4.1.5.tgz
```
...
```sh
helm upgrade my-release -f values.yaml ibm-sccm-4.1.5.gz
```
...
2. Data in motion is encrypted using transport layer security(TLS 1.2). For more information please see product [Knowledge center link]( https://www.ibm.com/docs/en/control-center/6.4.1?topic=sterling-control-center-monitor-641 )
